...
from bbq_sample import load_jsonl, build_items, CLEAN_PATH
from client import Qwen3Client
from eval import format_prompt, get_answer_metadata, parse_answer
from metrics import keyword_terms, find_mentions

MODEL = "qwen3.5:9b"
EFFORTS = ["low", "medium", "high"]
SAMPLE_PAIRS = 5
SEED = 0
# Draw the case-study examples from the same frozen, opposite-alignment
# subset the main sweep uses (data/build_clean_pairs_subset.py) rather than
# the raw twin pool - keeps this a small, separate qualitative case study
# (per feedback_points_pratham.md, Mentor 1) while guaranteeing every
# example pair actually tests commitment asymmetry, and picks up each
# pair's ambiguous sibling for free via bbq_sample.build_items.
PAIRS_SUBSET_PATH = (
    Path(__file__).resolve().parent.parent
    / "data"
    / "bbq_pairs_subset_700_clean.jsonl"
)
...
def main():
    ...
    twins = load_jsonl(PAIRS_SUBSET_PATH)
